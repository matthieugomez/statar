...
#' # Display format depends on period
#' datem
#' as.character(datem)
#' # Internally stocked as inter, which allows things such as:
#' datem + 1
...
#' week(datem)
#' @details Weekly, monthly and quarterly dates are stored as the number of elapsed calendar periods since 01/01/1970. This allows to use arithmetic on dates other than daily. Moreover, these dates are printed in a way that fits their frequency  (\code{YYY}q\code{q}, \code{YYY}m\code{MM}, \code{YYY}w\code{WW}).
#'
