...
#' @param time Numeric variable along which gaps should be filled. Default to last key. ee the \link[dplyr]{select} documentation.
...
fill_gap_ <- function(x, ..., .dots, full = FALSE, roll = FALSE, rollends = if (roll=="nearest") c(TRUE,TRUE)
	...
	if (!full){
		...
		a <- min(x[[timevar]])
		b <- max(x[[timevar]])
		ans <- x %>% do(setNames(data_frame(datem = seq(a, b, by = 1)), timevar))
	}
	setDT(ans)
	setDT(x)
	...
	# set the key and merge with roll
