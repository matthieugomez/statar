...
#' @param type type of graph among "density", "boxplot", "line", "felm", 
...
#' @export
graph <- function(x, ..., along_with = NULL, by = NULL, w = NULL, reorder = TRUE, winsorize = TRUE, facet = FALSE, verbose = FALSE, type = if (is.null(substitute(along_with))){"density"} else {"felm"}, formula = NULL, n = 20) {
...
#' @export
#' @rdname graph
graph_<- function(x, ..., .dots , along_with = NULL, by = NULL, w = NULL, reorder = TRUE, winsorize = TRUE , facet = FALSE, verbose = FALSE, type = if (is.null(along_with)){ "density"} else {"felm"}, formula = NULL, n = 20){
  stopifnot(is.data.table(x))
  type <- match.arg(type, c("density", "boxplot", "line", "felm"))
        ...
        if (length(along_with)){
          if (type == "line"){
            ...
            ans[, c(v, along_with) := { 
              ...
              out_v <- mean(.SD[[v]], na.rm = TRUE) + residuals(felm(formula_v, .SD))
              formula_along_with <- as.formula(paste0(along_with, deparse(formula)))
              out_along_with <- mean(.SD[[v]], na.rm = TRUE) + residuals(felm(formula_along_with, .SD))
              list(as.vector(out_v), as.vector(out_along_with))
            }
            , by = c(group), 
            .SDcols = setdiff(names(ans), group)
            ]
            ans_coeff <- ans[, as.list(coef(felm(as.formula(paste0(v,"~", along_with)), .SD))), by = c(group), .SDcols = setdiff(names(ans), group)]
