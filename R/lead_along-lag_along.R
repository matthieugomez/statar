...
#' @param units computes lag based on "day", "week", "month", "quarter" or "year" periods. Should be used when \code{date} is a date. 
#' @param date specifes the date variable
...
#' DT %>% group_by(id) %>% mutate(lag_along(value, 1, units = "month", along_with = date)) 
...
#' @export
#' @rdname lead_along-lag_along
lag_along <- function(x, n = 1L, units = NULL, date, default = NA) {
    if (!is.null(units)) {
      if (is.null(date)) stop("units cannot be used without order_by")
      unitsc <-match.arg(units,c("day","week","month","quarter","year"))
      date_origin <- as.Date('0001-01-01')
