...
#' @param by Groups within which gaps should be fill. Default to keys (or to keys minus last if along_with is unspecified)
#' @param along_with Numeric variable along which NAs should be filled. Default to last key.
...
#' @export
setna <- function(.data, ..., by = NULL, along_with = NULL, roll = TRUE ,  rollends = if (roll=="nearest") c(TRUE,TRUE)
  else if (roll>=0) c(FALSE,TRUE)
  else c(TRUE,FALSE)){
  	setna_(.data, .dots = lazy_dots(...) , by = substitute(by), along_with = substitute(along_with), roll = roll, rollends = rollends)
}
