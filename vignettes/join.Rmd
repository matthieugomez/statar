...
It corresponds roughty to the Stata command `reclink`, except that rows in `y` are not thrown away after an exact match with `x`, that the type of distance between strings can be arbitrarly specified, and that it is faster.
...
- Kind of string distance used (any distance available in `stringdist::strindist` works).

```R
x <- data.table(a = c("france", "franc"), b = c("arras", "dijon"))
y <- data.table(a = c("franc", "france"), b = c("arvars", "dijjon"))
fuzzy_join(x, y, fuzzb = c("a", "yb"))
fuzzy_join(x, y, fuzzb = c("a", "yb"), w = c(0.9, 0.1))
fuzzy_join(x,y, fuzzb = c("a", "yb"), w = c(0, 0.9))
x <- data.table(a = c(1, 1), b = c("arras", "dijon"))
y <- data.table(a = c(1, 1), b = c("arvars", "dijjon"))
fuzzy_join(x, y, exact = "a", fuzzb = "yb")
x <- data.table(a = c(1, 2), b = c("arras", "dijon"))
y <- data.table(a = c(1, 1), b = c("arvars", "dijjon"))
fuzzy_join(x,y, exact = "a", fuzzy = "yb")
```
