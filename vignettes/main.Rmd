...
- The functions `pastem`, which corresponds to R `paste`, implements string interpolations similarly to Stata and Julia.
...
- The function `quotem` implements expression interpolations. Note that names starting with the pattern `$`  must be enclosed in backquotes to be considered valid names in R.
  ...
  #> DT[, list(temp = mean(v1)), by = c("id", "v1")]
  ```

  The function `evalm` is a wrapper for `eval(quotem())`. The function corresponds to Julia `@eval` and to Stata natural macros (although restricted to name or object substitutions).
...
|Stata     | statar)      
|:-----------|------------
|merge v1, keep(master matched using)   | join(x, y, kind = "outer") 
...
The package also includes three functions can then be used on daily dates,  years, or elapsed dates (see ?elapsed)
