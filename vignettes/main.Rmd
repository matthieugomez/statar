...
The function `quotem` also subsitute expressions.
...
```
Note that, with the pattern "$", names need to be backquoted so that R accepts these expressions as valid names.  The option pattern allows to use a different pattern. 
```R
quotem("My record indicates you are #height inches tall", pattern = "#")
#> [1] "My record indicates you are 72 inches tall"
```
If the pattern is common, you can use the option `parenthesis.only = TRUE` to replace only expressions following the pattern enclosed in parenthesis.
```R
