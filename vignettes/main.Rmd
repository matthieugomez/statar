...
The functions `quotem` implements expression and string interpolations, similarly to Julia and to Stata macros.
...
The function `quotem` also works with expressions.
...
```
With the pattern `$`, names need to be backquoted to be accepted by R  as valid names. The function `evalm` is a wrapper for `eval(quotem())` (and therefore corresponds to Julia `@eval` and to Stata natural macros)
...
The package makes working with panel data easier in R. 
