...
roll applies a function along a time variable

```R
year <- c(1989, 1991, 1992)
value <- c(1, 1, 1)
roll(value, sum, n = 2, order_by = year) # based on the two previous rows
roll(value, sum, n = 2, along_with = year) # based on [year-2, year]
```

These functions can be used within a call to `mutate` or `data.table`, ie "by" group
