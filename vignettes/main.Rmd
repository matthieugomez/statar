...
#> [1] "My record indicates you are 72 inches tall"
```
If the pattern is common, you can use the option `parenthesis.only = TRUE` to replace expressions enclosed in parenthesis only. Note that only the first dot is replaced:
```R
...
```
Note that, with the pattern `$`, names need to be backquoted to be accepted by R  as valid names.  
