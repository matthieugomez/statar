...
The classes "monthly", "quarterly"  print as dates and are compatible with usual time extraction (ie `month`, `year`, etc). Yet, they are stored as integers representing the number of elapsed periods since 1970/01/0 (resp in week, months, quarters). This is particularly handy for simple algebra:
...
`lag`/`lead` create lag/lead variables along a time variable
...
These functions can be applied to any vector. In particular, they are intended to be used within `mutate` or `data.table`, i.e. within groups.
