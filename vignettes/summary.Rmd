...
keep and drop create a new table with certain columns
```R
DT <- data.table(
  id = c(1,2),
  v1 = c(1,1),
...
`keep_if` and `drop_if` create a new table with certain rows
```R
DT <- data.table(
  id = c(1,2,1),
  v1 = c(1,NA,2)
...
`drop_if(condition)` differs from `keep_if(!(condition))`: it keeps rows where the condition evaluates to `NA`.
### Summarize
